...
size_t input_rank(RANK** ppRank, size_t old_count)
{
	...
	//기존 배열의 유무에 따라 1개 또는 기존배열 개수 + 1 로 new_count를 설정 
	...
	RANK tmp;	//입력값을 임시 저장할 지역변수

	printf("직급 코드를 입력하세요 : ");
	scanf("%hd", &tmp.id);

	char ch;
	scanf("%c", &ch);	//입력 버퍼에 남아 있는 \n 삭제하기. (Enter 삭제)
	...
	if (pRank)	//기존에 배열이 있다면
	{
		...
		memcpy(pNew, pRank, old_size);	//기존 배열을 새 배열에 복사
	}

	//입력받은 데이터를 새 배열 마지막에 복사
...
int update_rank(RANK* pRank, size_t count)
{
	...
	//입력 받은 코드를 이용하여 해당 배열에서 데이터의 인덱스를 구한다. 
...
size_t load_rank(RANK** ppRank)
{
	...
	fseek(fp, 0, SEEK_SET);	//다시 BOF 지점으로 돌려놓음
...
size_t input_part(PART** ppPart, size_t old_count)
{
	...
	if (pPart)	//기존에 배열이 있다면
	{
		...
		memcpy(pNew, pPart, old_size);	//기존 배열을 새 배열에 복사
	}
...
void save_part(const PART* pPart, const size_t count)
{
	if (!pPart || !count)
	{
		//파일 삭제
		char cmd[30] = { 0 };
		sprintf(cmd, "del %s", PART_FILE);	//del은 도스용 삭제 언어, rm은 리눅스용, PART_FILE 
		system(cmd);
	}
	...
	const PART* pPart, size_t count_part, const RANK* pRank, size_t count_rank)
{
	...
	EMPLOYEE emp = { 0 };	//문자열 입력을 받기 위해 모든 값을 0으로 초기화하는게 좋다. 
	printf("사원 코드를 입력하세요 >>> ");
	scanf("%hd", &emp.id);
	...
	size_t new_bytes = new_count * sizeof(EMPLOYEE);
	...
	{
		//기존 데이터가 존재할 경우
		memcpy(pNewEmp, pOldEmp, sizeof(EMPLOYEE) * sizeof(count_emp));	//바이트 단위로 복사
	}
...
size_t load_employee(EMPLOYEE** ppEmp)
{
	...
	FILE* fp = NULL;
	fp = fopen(EMPL_FILE, "rb");

	if (!fp)
	{
		printf("사원 파일을 열 수 없습니다.");
		return 0;
	}

	fseek(fp, 0, SEEK_END);
	long size = ftell(fp);
	fseek(fp, 0, SEEK_SET);	//처음 위치로 커서를 되돌림
