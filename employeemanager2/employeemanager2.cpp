...
{
	printf("메뉴를 선택하세요 >>>");
	return _getch();	//conio.h에 정의된 기능, 키 입력시 즉시 반환함
...
//void print_data(int a); //원형 정의(실제로 정의되지 않으면 빈 박스)
//코드가 문법에만 맞으면 컴파일 에러는 뜨지 x, but 빈 박스는 빌드 에러가 뜸
int main()
{
	...
	do
	{
		...
		case '3': // 사원 정보 수정
			...
			save_employee(pEmp, count_emp);	//변경 내용 저장
			print_employee(pEmp, count_emp, pPart, count_part, pRank, count_rank);	//변경 내용 출력
			break;
		case '4': // 사원 정보 삭제
		...
		case '7': // 직급 정보 수정
			...
			break;
		case '8': // 직급 정보 삭제
			count_rank = delete_rank(&pRank, count_rank);	//run-time error 발생
			save_rank(pRank, count_rank);
			print_rank(pRank, count_rank);
			break;
		case 'a': // 부서 정보 입력
